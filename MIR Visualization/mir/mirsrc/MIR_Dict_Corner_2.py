...
# Corner 2 is the top left corner.

def CalCorner_2(blockName, job_dta, W, L, tree):
	feasibility, potChain = Feasibility_C2(blockName, job_dta, tree)
	if feasibility:
		# print 'Check point 1'
		treeAfter = DuplicateTree(tree)
		treeAfter = UpdateTree_C2(blockName, job_dta, treeAfter)
...
def PotentialChain_C2(blockName, tree):
	coord = BlockToCoord(blockName)
	ceiling = coord[3]
	# print 'checkpoint 1'
	return PC_C2_Rec(blockName, ceiling, tree)


def PC_C2_Rec(blockName, ceiling, tree):
	coord = BlockToCoord(blockName)
	if not tree[blockName][1]:
		...
		for b in tree[blockName][1]:
			...
			if ceiling <= nextcoord[3] and ceiling > nextcoord[2]:
				# print 'checkpoint 4'
				return [blockName] + PC_C2_Rec(b, ceiling, tree)
		return [blockName]
...
def Feasibility_C2(blockName, job, tree):
	w, l = job.dimension
	...
	potChain_all = PotentialChain_C2(blockName, tree)
	rightPoint = x1 + w
...
def UpdateTree_C2(startBlockName, job, tree):
	feasibility, influencedChain = Feasibility_C2(startBlockName, job, tree)
	if not feasibility:
		print 'Warning(UpdateTree_C2.w1): Feasiblity is False, no updating required.\n'
		return 

	coord = BlockToCoord(startBlockName)
	w, l = job.dimension
	corner = [coord[0], coord[3]]
	lenInfluencedChain = len(influencedChain)
	tempTree = DuplicateTree(tree)
	jobCoverCoord = [corner[0], corner[0] + w, corner[1] - l, corner[1]]
	# print 'job Cover Coord:', jobCoverCoord
	for i, b in enumerate(influencedChain):
		if i == 0:
			UpdateTree_C2_T1(b, jobCoverCoord, tempTree)
		elif i > 0 and i < (lenInfluencedChain - 1):
			UpdateTree_C2_T2(b, jobCoverCoord, tempTree)
		elif i == (lenInfluencedChain - 1):
			UpdateTree_C2_T3(b, jobCoverCoord, tempTree)
	for b in influencedChain:
		DeleteBlock(b, tempTree)
	return tempTree

def UpdateTree_C2_T1(b, jobCoverCoord, tree):
	...
	# The top left corner of the job and the starting block should overlap
	if x1 != jx1 or y2 != jy2:
		print 'Error(UpdateTree_C2_T1.e1): input should be the starting block, but it does not agree with the corner coordinates. \n'
		return
	
	# The bottom of the job should not be lower than the bottom of the block
	if jy1 < y1:
		print 'Error(UpdateTree_C2_T1.e2): The bottom of the job exceeds the bottom of the block.'
		return

	# """ Case 1: Total Consumption """
	# If the job happens to consume all of the block, then delete the block from the tree
	if jx2 >= x2 and jy1 == y1:
...
def UpdateTree_C2_T2(b, jobCoverCoord, tree):
	...
	# Check input 
	if jx1 >= x1 or jx2 <= x2:
		print 'Error(UpdateTree_C2_T2.e1): jobCoverCoord does not exceed block width.'
		return
	elif jy1 < y1 or jy2 > y2:
		print 'Error(UpdateTree_C2_T2.e2): jobCoverCoord exceeds block height.'
		return
	...
	# """ Case 4: Consume middle part """
		...
		print 'Error(UpdateTree_C2_T2.e3): Unknown Error.'
		return


def UpdateTree_C2_T3(b, jobCoverCoord, tree):
	x1, x2, y1, y2 = BlockToCoord(b)
	jx1, jx2, jy1, jy2 = jobCoverCoord

	# Check input
	if jx1 >= x1 or jx2 > x2:
		print 'Error(UpdateTree_C2_T3.e0): jobCoverCoord width error.'
		return
	elif jy1 < y1 or jy2 > y2:
		print 'Error(UpdateTree_C2_T2.e2): jobCoverCoord exceeds block height.'
		return
